...
export default function CityClock({ cityName, timezone, countryCode = "US", onOpenOnThisDay, onOpenHoliday }: CityClockProps) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (text) {
            ...
            // Adjust 'today' to the local timezone of the city so it aligns correctly
            const todayIso = new Date(new Date().toLocaleString('en-US', { timeZone: timezone })).toISOString().split('T')[0];
  ...
  return (
    <section className="bg-black text-white rounded-[14px] p-6 md:px-9 md:py-5 grid grid-cols-1 md:grid-cols-[1.4fr_1fr_1.4fr] items-center gap-6 md:gap-0 shadow-[0_10px_40px_rgba(0,0,0,0.7)] min-h-[135px] mb-2.5 border border-white/10">
      <div className="flex flex-col items-center md:items-start text-center md:text-left">
        <div className="flex items-center gap-3 mb-2 md:mb-1">
          ...
          <div className="inline-flex border-[1.5px] border-white rounded-full overflow-hidden h-6">
            <button
        ...
        <div className="text-[50px] md:text-[62px] font-bold leading-[0.9] md:leading-[0.8] tracking-[-0.04em] text-white flex items-end justify-center md:justify-start">
          <span>{display.hour}:{display.minute}</span>
          <span className="text-[18px] md:text-[22px] text-white ml-2 md:ml-2.5 font-medium mb-1 md:mb-0">{display.second}</span>
          {hour12 && <span className="text-[16px] md:text-[20px] ml-2 font-medium text-white mb-1 md:mb-0">{display.dayPeriod}</span>}
        </div>
        ...
        {holiday && (
          <button 
            ...
            style={{ 
              marginTop: "16px", background: "none", border: "none", padding: 0, cursor: "pointer",
              fontSize: "18px", fontWeight: 900, color: "#3b82f6", textTransform: "uppercase", letterSpacing: "0.15em",
      ...
      <div className="flex flex-col items-center gap-1 mt-2 md:mt-0">
        <div className="text-[16px] md:text-[18px] font-normal text-white uppercase tracking-[0.2em]">{dayName}</div>
        <div className="text-[12px] md:text-[14px] font-bold text-white uppercase tracking-[0.1em] mb-2">{display.fullDate}</div>
        <button
          onClick={() => {
      ...
      <div className="hidden md:block justify-self-end relative w-[120px] h-[120px]">
        <div style={{ position: "relative", width: "120px", height: "120px", borderRadius: "50%", border: "2.5px solid #ffffff", background: "#0f172a" }}>
          {[12, 3, 6, 9].map((num) => (
            <span key={num} style={{
              position: "absolute", fontSize: "11px", fontWeight: 900, color: "#ffffff",
          ...
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} style={{ 
                ...
                width: "1.5px", height: i % 3 === 0 ? "10px" : "6px", 
                background: i % 3 === 0 ? "#ffffff" : "#475569", 
          ...
          <div style={{ position: "absolute", left: "50%", top: "50%", width: "1.5px", height: "48px", background: "#3b82f6", borderRadius: "999px", transform: `translate(-50%, -100%) rotate(${analog.secondAngle}deg)`, transformOrigin: "bottom center" }} />
